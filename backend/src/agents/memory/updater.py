...
def _load_memory_from_file() -> dict[str, Any]:
    ...
    try:
        ...
        logger.error("Failed to load memory file: %s", e, exc_info=True)
        return _create_empty_memory()
...
def _save_memory_to_file(memory_data: dict[str, Any]) -> bool:
    ...
    try:
        ...
        logger.error("Failed to save memory file: %s", e, exc_info=True)
        return False
...
class MemoryUpdater:
    ...
    def update_memory(self, messages: list[Any], thread_id: str | None = None) -> bool:
        ...
        try:
            ...
            # Save
            saved = _save_memory_to_file(updated_memory)
            if saved:
                try:
                    from src.vector import index_memory

                    index_memory(updated_memory)
                except Exception as exc:
                    logger.warning("Failed to index memory vectors: %s", exc, exc_info=True)
            return saved

        except json.JSONDecodeError as e:
            logger.error("Failed to parse LLM response for memory update: %s", e, exc_info=True)
            return False
        except Exception as e:
            logger.error("Memory update failed: %s", e, exc_info=True)
            return False

...
class LocalSandboxProvider(SandboxProvider):
    ...
    def acquire(self, thread_id: str | None = None) -> str:
        ...
        # Dynamically map /mnt/user-data to the thread's local data directory
        if thread_id:
            user_data_path = os.path.join(
                os.getcwd(), ".deer-flow", "threads", thread_id, "user-data"
            )
            for subdir in ("uploads", "outputs", "workspace"):
                os.makedirs(os.path.join(user_data_path, subdir), exist_ok=True)
            _singleton.path_mappings["/mnt/user-data"] = user_data_path

...
class AppConfig(BaseModel):
    ...
    def from_file(cls, config_path: str | None = None) -> Self:
        ...
        # Load uploads processing config if present
        if "uploads" in config_data:
            load_uploads_config_from_dict(config_data["uploads"])

        # Load vector config if present
        if "vector" in config_data:
            load_vector_config_from_dict(config_data["vector"])

        # Load ops config if present
        if "ops" in config_data:
            load_ops_config_from_dict(config_data["ops"])

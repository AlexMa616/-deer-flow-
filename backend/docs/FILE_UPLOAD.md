...
- ✅ 支持多文件同时上传
- ✅ 自动转换文档为 Markdown（PDF、PPT、Excel、Word，异步处理）
- ✅ 异步内容分析（摘要、关键词）
- ✅ 文件存储在线程隔离的目录中
- ✅ 返回内容类型、校验和与文本预览（适用于文本文件）
- ✅ Agent 自动感知已上传的文件
- ✅ 支持文件列表查询和删除
- ✅ 支持处理状态查询（队列、进度、摘要、日志）
- ✅ 支持 SSE 实时推送状态/日志（替代轮询）
- ✅ 支持处理任务取消与重试
  ...
  "files": [
    {
      ...
      "processing_job_id": "job_abcd1234",
      "processing_status": "queued",
      "processing_progress": 0,
      "content_type": "application/pdf",
      "checksum_sha256": "b5d4045c3f466fa91fe2cc6abe79232a1a57cdf104f7a26e716e0a1e2789df78",
      "preview": null,
...
### 3. 查询处理状态
```
GET /api/threads/{thread_id}/uploads/status?filename={filename}
```
...
[
  {
    ...
    "status": "processing",
    "progress": 60,
    "steps": {"convert": "completed", "analyze": "running", "index": "pending"},
    "summary": "简要摘要...",
    "keywords": ["关键词1", "关键词2"],
    "language": "Chinese",
    "highlights": ["要点1", "要点2"],
...
### 4. 实时状态推送（SSE）
```
GET /api/threads/{thread_id}/uploads/stream?filename={filename}
```

### 5. 取消处理任务
```
POST /api/threads/{thread_id}/uploads/cancel
```

### 6. 重试处理任务
```
POST /api/threads/{thread_id}/uploads/retry
```
...
转换后的 Markdown 文件会保存在同一目录下，文件名为原文件名 + `.md` 扩展名（由后台异步生成）。

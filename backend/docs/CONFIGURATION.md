...
Asynchronous upload conversion and content analysis:
...
Semantic search for uploads and memory:

```yaml
vector:
  enabled: true
  storage_path: .deer-flow/vector-index.duckdb
  incremental: true
  dedupe: true
  batch_size: 32
  parallel_workers: 2
  parallel_min_chunks: 24
  embedding:

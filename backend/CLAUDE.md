...
| **Uploads** (`/api/threads/{id}/uploads`) | `POST /` - upload files; `GET /list` - list; `GET /status` - processing status; `DELETE /{filename}` - delete |
| **Artifacts** (`/api/threads/{id}/artifacts`) | `GET /{path}` - serve artifacts; `?download=true` for file download |
| **System** (`/api/system`) | `GET /overview` - aggregated stats + recommendations |
| **Semantic** (`/api/semantic`) | `GET /search` - vector search; `POST /reindex` - rebuild index |

Note: `/api/system/overview` uses an 8-second in-memory cache to avoid repeated filesystem and DuckDB scans during rapid UI refreshes. Cache definitions live below response models to avoid forward-reference errors in Python.
...
- Filenames are sanitized to prevent path traversal; uploads include content type + checksum metadata, async conversion + analysis status, and are persisted locally and mirrored to remote sandboxes when applicable

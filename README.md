...
Uploads are stored per-thread with filename sanitization to prevent path traversal, and processed asynchronously for conversion + analysis. Local copies are retained so artifacts stay accessible across sandbox modes.

The backend also exposes `/api/system/overview` and `/api/system/metrics` for aggregated operational stats, plus `/api/semantic/search` for semantic retrieval with citations.
